        ...
        <tbody class="divide-y divide-slate-50">
            ...
            <tr class="group hover:bg-blue-50/20 transition-colors">
                ...
                <td class="px-6 py-4 text-center">
                    ...
                    @else
                        <span class="px-2 py-1 rounded bg-slate-50 text-slate-600 text-[10px] font-black uppercase tracking-tighter">{{ ucfirst($withdraw->status) }}</span>
                    @endif
                </td>
                <td class="px-6 py-4 text-right">
                    <div class="flex items-center justify-end gap-2">
                        <button type="button"
                                data-withdraw="{{ json_encode($withdraw->load('campaign')) }}"
