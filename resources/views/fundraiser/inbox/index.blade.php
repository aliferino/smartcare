@extends('layouts.panel', ['title' => 'Inbox'])

@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-black text-slate-900 uppercase">Inbox</h1>
    <p class="text-slate-500 text-xs font-medium uppercase tracking-widest opacity-70">Announcements from admin</p>
</div>
...
<div class="mb-6">
    <div class="relative">
        <input type="text" id="searchInput"
            placeholder="Type to search messages and press Enter..."
...
<div id="table-container" class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
    <table class="w-full">
        ...
        <tbody class="divide-y divide-slate-100">
            ...
            <tr onclick="openBroadcastModal({{ $broadcast->id }})" class="hover:bg-blue-50 transition-colors cursor-pointer">
                ...
                <td class="px-6 py-4 text-slate-600 text-sm">{{ $broadcast->sent_at->format('d M Y, H:i') }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="4" class="px-6 py-12 text-center">
                    ...
                    <p class="text-sm text-slate-500 mt-2">You'll receive announcements from admin here</p>
                </td>
            </tr>
            @endforelse
        </tbody>
...
@include('fundraiser.inbox._modal')
@endsection
