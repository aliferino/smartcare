...
<script>
    ...
    function populateCampaignDetail(campaign) {
        ...
        const html = `${imageHtml}<div class="flex items-start justify-between mb-6"><div><h4 class="text-xl font-black text-slate-900 uppercase tracking-tight mb-2">${campaign.title}</h4><p class="text-xs font-bold text-blue-600 uppercase tracking-widest">${campaign.campaign_category.name}</p></div><div class="flex flex-col items-end gap-2">${statusBadge}${urgentBadge}</div></div><div class="p-6 rounded-2xl bg-slate-50 border border-slate-100 space-y-6"><div class="text-center"><label class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2 italic">Campaign Period</label><div class="flex items-center gap-4"><div class="flex-1"><p class="text-[8px] font-black text-slate-400 uppercase">Start Date</p><p class="text-xs font-bold text-slate-700">${startDate}</p></div><div class="w-px h-8 bg-slate-200"></div><div class="flex-1"><p class="text-[8px] font-black text-slate-400 uppercase">End Date</p><p class="text-xs font-bold text-slate-700">${endDate}</p></div></div></div><div class="pt-4 border-t border-slate-200/60"><div class="flex justify-between items-end mb-2"><p class="text-xs font-black text-slate-900 uppercase tracking-tighter"><span>IDR ${Number(campaign.current_amount).toLocaleString('id-ID')}</span> <span class="text-[9px] text-slate-400 font-bold ml-1">Collected</span></p><p class="text-[10px] font-bold text-slate-400 italic">Target: IDR ${Number(campaign.goal_amount).toLocaleString('id-ID')}</p></div><div class="w-full bg-white rounded-full h-2.5 shadow-inner overflow-hidden border border-slate-200/50"><div class="bg-blue-600 h-full rounded-full transition-all duration-1000" style="width: ${percentageRounded}%"></div></div><div class="flex justify-end mt-1"><span class="text-[10px] font-black text-blue-600">${percentageRounded}%</span></div></div></div><div class="border-t border-slate-100 pt-6"><p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Entity Information</p><div class="flex items-start gap-4 p-4 bg-slate-50 rounded-xl">${entityLogo}<div class="flex-1"><p class="text-sm font-black text-slate-900 uppercase tracking-tight mb-1">${campaign.entity.name}</p><p class="text-xs text-slate-600 mb-1">${campaign.entity.email}</p><p class="text-xs text-slate-500">${campaign.entity.address}</p></div></div></div><div class="border-t border-slate-100 pt-6"><p class="text-xs font-bold text-slate-500 uppercase tracking-wider mb-3">Description</p><div class="p-4 bg-slate-50 rounded-xl"><p class="text-sm text-slate-700 leading-relaxed whitespace-pre-line">${campaign.description}</p></div></div>${rejectionHtml}`;
