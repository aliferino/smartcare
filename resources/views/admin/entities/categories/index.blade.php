@extends('layouts.panel', ['title' => 'Entity Categories'])

@section('content')
<div class="mb-6 flex justify-between items-end">
    <div>
        <h1 class="text-2xl font-black text-slate-900 uppercase">Entity Categories</h1>
        <p class="text-slate-500 text-xs font-medium uppercase tracking-widest opacity-70">Manage organization classification types</p>
    </div>
...
<div id="table-container" class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
    @include('admin.entities.categories._table', ['categories' => $categories])
</div>
...
<script>
    ...
    function loadPage(page = 1) {
        ...
        $.ajax({
            url: '{{ route("admin.entities.categories.index") }}',
            method: 'GET',
            data: { page: page },
            success: function(response) {
                ...
                // Keep URL clean
                history.replaceState(null, '', '{{ route("admin.entities.categories.index") }}');
            },
            error: function() {
                container.css('opacity', '1');
    ...
    $('#categoryForm').on('submit', function(e) {
        ...
        let url = method === 'edit'
            ? `/admin/entities/categories/${categoryId}/update`
            : '{{ route("admin.entities.categories.store") }}';
    ...
    function deleteCategory(id) {
        ...
        $.ajax({
            url: `/admin/entities/categories/${id}/delete`,
            type: 'POST',
            data: {
                _token: '{{ csrf_token() }}',
                _method: 'DELETE'
